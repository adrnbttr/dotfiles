...
# Ouvre un onglet kitty par worktree data-engineering (dossiers de-* qui
# partagent le .git du dépôt principal). Titre de l'onglet = nom du worktree.
# Les worktrees mergés/supprimés disparaissent naturellement de la session.
de_session() {
  local wt_root="${DE_WT_DIR:-$HOME/Documents/github/marvin/data-engineering-wt}"
  local main_repo="${DE_MAIN_DIR:-$HOME/Documents/github/marvin/data-engineering}"
  local state_dir="${XDG_STATE_HOME:-$HOME/.local/state}/kitty"
  local session="$state_dir/de-session.conf"

  if [[ ! -d "$wt_root" ]]; then
    echo "de-session: directory not found: $wt_root" >&2
    return 1
  fi
  ...
  if (( ${#repos} == 0 )); then
    echo "de-session: aucun worktree sous $wt_root (et pas de dépôt principal à $main_repo)" >&2
    return 1
  fi
  ...
  if [[ -n "${KITTY_WINDOW_ID:-}" ]] && kitty @ ls >/dev/null 2>&1; then
    ...
    if (( created > 0 )); then
      ...
      echo "de-session: échec de création des onglets via kitty remote control" >&2
      return 1
    fi
  ...
  {
    echo "# Autogenerated by de-session"
