...
- Recent edits are briefly highlighted (10s) so you can spot changes from tools like opencode/avante.
...
avante.nvim:
  <leader>aa    ask (chat on current context)
  <leader>an    new chat
  ...
  <leader>aj    next prompt in sidebar
  <leader>ak    previous prompt in sidebar
  <M-l>         accept inline suggestion (Avante)
  <M-j>         next inline suggestion (Avante)
  <M-k>         previous inline suggestion (Avante)
  <M-h>         dismiss inline suggestion (Avante)
#+end_src
