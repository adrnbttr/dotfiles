...
*Clavier BÉPO sur un portable AZERTY (Windows)* : `windows/clavier/` ajoute la
BÉPO native de Windows à côté du clavier Belge imprimé (Win+Espace pour
basculer), avec de petits correctifs pour qu'elle tape comme la BÉPO Linux
(apostrophe, underscore). Une commande dans PowerShell :
...
(sous Windows : =%USERPROFILE%\.config\wezterm\keyboard=) force le choix.
