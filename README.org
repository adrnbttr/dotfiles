...
./script/install --claude-version 2.1.72
#+end_src
...
Sessions persistantes (Linux) : shells, nvim et Claude Code tournent dans
`wezterm-mux-server`. Fermer la fenêtre ou relancer WezTerm ne tue rien :
`wezterm` (ou le lanceur du menu, surchargé dans
`bin/.local/share/applications/`) se reconnecte. Tout arrêter pour de bon :
`pkill -x wezterm-mux-ser`. Un changement de `config.term` ou des domaines
ne s'applique qu'après ce redémarrage du serveur.

Workspaces : `de-session`, `ms-session`, `github-session` et
`marvin-session` ouvrent chacune leur workspace (de, ms, github, marvin) ;
les relancer n'ajoute que les onglets manquants.
...
splits:
  ...
  Ctrl+Alt+Arrow keys    focus split (pris par Cinnamon s'il a plusieurs bureaux)
  Ctrl+Alt+a / x         aller à / échanger un split par lettre (a u i e t s r n)
  Ctrl+b                 split précédent
  Ctrl+Alt+r             resize mode (flèches, Échap)
  Ctrl+Alt+z             zoom

tabs:
  Ctrl+Alt+t / w         new / close tab
  Ctrl+Tab, Ctrl+Shift+Right   next tab
  Ctrl+Shift+Tab, Ctrl+Shift+Left  previous tab
  Ctrl+Shift+PageUp/Down move tab
  Ctrl+Shift+o           tab navigator (fuzzy)

workspaces:
  Ctrl+Shift+s           workspace launcher (fuzzy)
